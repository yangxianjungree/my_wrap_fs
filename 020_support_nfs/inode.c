...
static int loopfs_setxattr(struct dentry *dentry, struct inode *inode,
				const void *name, const void *value, size_t size, int flags)
{
...
static ssize_t loopfs_getxattr(struct dentry *dentry, struct inode *inode,
...
static int loopfs_removexattr(struct dentry *dentry, struct inode *inode, const char *name)
{
	int err;
	struct dentry *lower_dentry;
	struct inode *lower_inode;
...
static int loopfs_xattr_get(const struct xattr_handler *handler,
			    struct dentry *dentry, struct inode *inode,
			    const char *name, void *buffer, size_t size)
{
	return loopfs_getxattr(dentry, inode, name, buffer, size);
}

static int loopfs_xattr_set(const struct xattr_handler *handler,
			    struct dentry *dentry, struct inode *inode,
			    const char *name, const void *value, size_t size,
			    int flags)
{
	if (value) {
		return loopfs_setxattr(dentry, inode, name, value, size, flags);
	}

	BUG_ON(flags != XATTR_REPLACE);
	return loopfs_removexattr(dentry, inode, name);
}

const struct xattr_handler loopfs_xattr_handler = {
	.prefix = "",		/* match anything */
	.get = loopfs_xattr_get,
	.set = loopfs_xattr_set,
};

const struct xattr_handler *loopfs_xattr_handlers[] = {
	&loopfs_xattr_handler,
	NULL
};