...
extern struct dentry *loopfs_lookup(struct inode *dir, struct dentry *dentry,
				unsigned int flags);
extern int loopfs_interpose(struct dentry *dentry, struct super_block *sb,
				struct path *lower_path);
extern int loopfs_is_blocked(struct loopfs_sb_info *sbinfo, const char *path,
				unsigned long inode);
...
extern const struct export_operations loopfs_export_ops;
extern const struct xattr_handler *loopfs_xattr_handlers[];
